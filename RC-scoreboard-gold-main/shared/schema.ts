...
export const insertTournamentPlayerStatSchema = createInsertSchema(tournament_player_stats);
export type InsertTournamentPlayerStat = z.infer<typeof insertTournamentPlayerStatSchema>;
...
export const insertTournamentHistorySchema = createInsertSchema(tournament_history);
...
export const insertPlayerCareerStatSchema = createInsertSchema(player_career_stats);
export type InsertPlayerCareerStat = z.infer<typeof insertPlayerCareerStatSchema>;
...
export const bets = pgTable(
	'bets',
	{
		...
		status: text('status')
			.notNull()
			.$type<'pending' | 'won' | 'lost' | 'push' | 'cancelled'>()
			.default('pending'),
...
export const parlays = pgTable(
	'parlays',
	{
		...
		status: text('status')
			.notNull()
			.$type<'pending' | 'won' | 'lost' | 'partial' | 'cancelled'>()
			.default('pending'),
		settledAt: timestamp('settled_at', { mode: 'string' }),
		createdAt: timestamp('created_at', { mode: 'string' }).defaultNow(),
		tournamentId: integer('tournament_id')
