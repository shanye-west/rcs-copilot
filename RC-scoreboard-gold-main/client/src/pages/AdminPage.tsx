...
function TournamentTab() {
	...
	const handleScoreChange = (
		...
		team: 'aviatorScore' | 'producerScore'
	) => {
		...
		<div className="space-y-4">
			<Card>
				<CardHeader>
					...
					<CardDescription>Configure the main tournament settings</CardDescription>
				</CardHeader>
				<CardContent>
					...
					<div className="flex space-x-4">
						<div className="bg-primary/10 flex-1 rounded-lg p-4">
							...
							<div className="space-y-4">
								...
								<div>
									...
									<p className="text-muted-foreground mt-1 text-xs">
										Score is automatically calculated from match results
									</p>
								</div>
		...
		<div className="space-y-4">
			...
			<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
				...
				{(!rounds || rounds.length === 0) && (
					<Card className="bg-muted/20 col-span-full">
			...
			{isEditDialogOpen && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
					<div className="bg-background max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg p-6 shadow-lg">
						<h2 className="mb-4 text-xl font-bold">Edit Round</h2>
						{roundFormJSX}
					</div>
				</div>
			)}
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
					<div className="bg-background w-full max-w-md rounded-lg p-6 shadow-lg">
						...
						<p className="text-destructive mb-4">
							Are you sure you want to delete this round? This will also delete all matches, scores,
							and player assignments associated with this round. This action cannot be undone.
						</p>
			...
			<div className="flex flex-col space-y-4">
				...
				<div className="bg-muted/50 border-border rounded-lg border p-4">
					<p className="mb-3 text-sm">
						A new dedicated player management page is now available with improved functionality for
						adding and deleting players.
					</p>
					<Button onClick={() => navigate('/admin/players')} variant="outline">
				...
				<Card key={team.id}>
					...
					<CardContent>
							...
							{playersByTeam[team.id]?.map((player: Player) => (
								...
								>
									...
									<div className="flex space-x-1">
										<Button variant="ghost" size="sm" onClick={() => handleOpenEditDialog(player)}>
											Edit
										</Button>
										<Button
											variant="ghost"
						...
						{(!playersByTeam[team.id] || playersByTeam[team.id].length === 0) && (
							<div className="text-muted-foreground py-4 text-center">No players in this team</div>
						)}
					</CardContent>
				</Card>
			))}
						...
						<tbody>
							{users?.map((user: User) => (
								<tr key={user.id} className="border-b">
									...
									<td className="p-3">
										<span
											className={`rounded-full px-2 py-1 text-xs ${user.isAdmin ? 'bg-primary/20' : 'bg-muted'}`}
										>
...
function ResetDeleteTab() {
	...
	const handleDeleteConfirmation = async (entity: 'rounds' | 'matches' | 'players' | 'scores') => {
		...
		switch (entity) {
			case 'rounds':
				action = async () => {
					await deleteAllRoundsMutation.mutateAsync();
				};
				title = 'Delete All Rounds';
				message =
					'Are you sure you want to delete all rounds? This will also delete all associated matches and scores. This action cannot be undone.';
				break;
			case 'matches':
				action = async () => {
				...
				message =
					'Are you sure you want to delete all matches? This will also delete all associated scores. This action cannot be undone.';
				break;
			case 'players':
				action = async () => {
			...
			case 'scores':
				action = async () => {
					await deleteAllScoresMutation.mutateAsync();
				};
				title = 'Delete All Scores';
				message =
					'Are you sure you want to delete all scores? This will reset all match statistics. This action cannot be undone.';
				break;
			default:
				return;
		}
	...
	const handleResetConfirmation = async (entity: 'rounds' | 'matches' | 'players') => {
		...
		switch (entity) {
			case 'rounds':
				action = async () => {
					await resetAllRoundsMutation.mutateAsync();
				};
				title = 'Reset All Rounds';
				message =
					'Are you sure you want to reset all rounds? This will set all rounds to their default state. This action cannot be undone.';
				break;
			case 'matches':
				action = async () => {
					await resetAllMatchesMutation.mutateAsync();
				};
				title = 'Reset All Matches';
				message =
					'Are you sure you want to reset all matches? This will set all matches to their default state and remove all scores. This action cannot be undone.';
				break;
			case 'players':
				action = async () => {
					await resetAllPlayersMutation.mutateAsync();
				};
				title = 'Reset All Players';
				message =
					'Are you sure you want to reset all player statistics? This will set all player wins, losses, and ties to zero. This action cannot be undone.';
				break;
			default:
				return;
		}
	...
	return (
		<div className="space-y-8">
			<div className="mb-6 rounded-md border border-red-200 bg-red-50 p-4">
				<h3 className="mb-2 text-lg font-bold text-red-800">⚠️ Danger Zone</h3>
				<p className="mb-2 text-sm text-red-700">
					The actions below will permanently delete or reset data in the tournament. These actions
					cannot be undone.
				</p>
			...
			{confirmationDialogOpen && confirmationAction && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
					<div className="bg-background w-full max-w-md rounded-lg p-6 shadow-lg">
						<h2 className="mb-2 text-xl font-bold">{confirmationAction.title}</h2>
