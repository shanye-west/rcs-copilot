...
const Match = ({ id }: { id: number }) => {
	...
	useEffect(() => {
		...
		if (
			...
			match.leadAmount > 18 - Math.max(...completedScores.map((s) => s.holeNumber), 0)
		) {
	...
	const handleEditMatchSubmit = async () => {
		...
		try {
			...
			// Players to add
			const newAviatorPlayers = selectedAviatorPlayers.filter(
				(p: any) => !existingAviatorPlayerIds.includes(p.id)
			);

			const newProducerPlayers = selectedProducerPlayers.filter(
				(p: any) => !existingProducerPlayerIds.includes(p.id)
			);

			// Players to remove
			const aviatorPlayersToRemove = participants.filter(
				(p: any) =>
					p.team === 'aviators' && !selectedAviatorPlayers.some((s: any) => s.id === p.playerId)
			);

			const producerPlayersToRemove = participants.filter(
				(p: any) =>
					p.team === 'producers' && !selectedProducerPlayers.some((s: any) => s.id === p.playerId)
			);
				...
				<>
					...
					<div className="mb-4 flex items-center justify-end">
						<div className="flex items-center space-x-2">
							{isAdminMode && (
								<div className="rounded bg-amber-100 px-3 py-1 text-xs font-medium text-amber-800">
									Admin View
								</div>
							)}

							{isAdmin && (
								<Button variant="outline" size="sm" onClick={handleToggleLock} className="ml-2">
									{isLocked ? (
										<>
						...
						<AlertDialogDescription>
							<div className="space-y-4">
								...
								<div className="flex items-center justify-between">
									<div className="bg-aviator rounded px-4 py-2 text-white">
										<p className="font-bold">The Aviators</p>
										<p className="text-center text-2xl">{matchSummary.aviatorTotal}</p>
									</div>
									<div className="text-xl font-bold">vs</div>
									<div className="bg-producer rounded px-4 py-2 text-white">
										<p className="font-bold">The Producers</p>
										<p className="text-center text-2xl">{matchSummary.producerTotal}</p>
									</div>
								</div>
			...
			<Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>Update the match details below.</DialogDescription>
					</DialogHeader>
