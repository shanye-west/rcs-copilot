				...
				{isAuthenticated && (
					<>
						<TabsContent value="my-bets" className="space-y-4">
												...
												<div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
													<div>
														...
														<div className="mt-1 flex items-center gap-2">
															<Badge className={getBetStatusColor(bet.status)}>{bet.status}</Badge>
													...
													<div className="text-right">
														<p className="text-sm">Wager: {formatCurrency(bet.amount)}</p>
							...
							<h2 className="mt-8 text-xl font-semibold">Bet History</h2>
							{userBets && userBets.length > 0 ? (
								<div className="grid grid-cols-1 gap-4">
									{userBets
										.filter((bet: any) => bet.status !== 'pending')
											...
											<Card key={bet.id} className="p-4">
												<div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
													<div>
														...
														{bet.actualResult && (
															<p className="mt-1 text-sm">Result: {bet.actualResult}</p>
														)}
													</div>
													<div className="text-right">
														<p className="text-sm">Wager: {formatCurrency(bet.amount)}</p>
										...
										<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
													...
													<FormItem>
														...
														>
															...
															<SelectContent>
																{betTypes &&
																	betTypes.map((betType: any) => (
																		<SelectItem key={betType.id} value={betType.id.toString()}>
																			{betType.description}
																		</SelectItem>
																	))}
															</SelectContent>
											...
											{selectedBetType && (
												<>
													...
													{['round_winner', 'over_under'].includes(selectedBetType.name) && (
														<FormField
															control={form.control}
															name="roundId"
															render={({ field }) => (
																...
																<Select onValueChange={field.onChange} defaultValue={field.value}>
																	...
																	<SelectContent>
																		{selectedBetType.name === 'match_winner' && (
																			<>
																				<SelectItem value="aviators">Aviators Win</SelectItem>
																				<SelectItem value="producers">Producers Win</SelectItem>
																				<SelectItem value="tie">Match Tie</SelectItem>
																			</>
																		)}
																		{selectedBetType.name === 'player_prop' && (
																			<>
																				<SelectItem value="win">Player Wins</SelectItem>
																				<SelectItem value="loss">Player Loses</SelectItem>
																				<SelectItem value="tie">Player Ties</SelectItem>
																			</>
																		)}
																		{selectedBetType.name === 'round_winner' && (
																			<>
																				<SelectItem value="aviators">Aviators Win Round</SelectItem>
																				<SelectItem value="producers">
																					Producers Win Round
																				</SelectItem>
																				<SelectItem value="tie">Round Tie</SelectItem>
																			</>
																		)}
													...
													<FormField
														...
														render={({ field }) => (
															<FormItem>
																...
																<FormControl>
																	<Input placeholder="Describe your bet" {...field} />
																</FormControl>
																<FormMessage />
															</FormItem>
														)}
														...
														<FormField
															...
															render={({ field }) => (
																<FormItem>
																	...
																	<FormControl>
																		<Input type="number" placeholder="Bet amount" {...field} />
																	</FormControl>
																	<FormMessage />
																</FormItem>
															)}
