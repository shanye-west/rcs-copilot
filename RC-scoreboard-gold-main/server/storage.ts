...
export interface IStorage {
	...
	updateCourseRatings(
		courseId: number,
		data: { courseRating: number; slopeRating: number; par: number }
	): Promise<any>;
...
export class DBStorage implements IStorage {
		...
		const result = await db.transaction(async (tx) => {
			...
			if (playerData.handicapIndex !== undefined && playerData.handicapIndex !== null) {
				...
				playerData.handicapIndex =
					typeof playerData.handicapIndex === 'string'
			...
			await tx.update(users).set({ playerId: player.id }).where(eq(users.id, user.id));
	...
	async updatePlayer(id: number, data: Partial<any>) {
		...
		if (data.name && updatedPlayer.userId) {
			...
			await db.update(users).set({ username: username }).where(eq(users.id, updatedPlayer.userId));
		}
	...
	async deletePlayer(id: number) {
		try {
			...
			if (player && player.userId) {
				...
				await db.update(users).set({ playerId: null }).where(eq(users.id, player.userId));

				// Delete associated match participants
				await db.delete(match_players).where(eq(match_players.playerId, id));
	...
	async getRounds() {
		...
		const roundsData = await db.select().from(rounds).where(isNull(rounds.status));
	...
	async updateRound(id: number, data: Partial<any>) {
		const [row] = await db.update(rounds).set(data).where(eq(rounds.id, id)).returning();
		return row;
	}
	...
	async deleteRound(id: number) {
		try {
			...
			await db.transaction(async (tx) => {
				...
				const roundMatches = await tx.select().from(matches).where(eq(matches.roundId, id));

				// For each match, delete scores and participants
				for (const match of roundMatches) {
					// Delete scores first
					await tx.delete(scores).where(eq(scores.matchId, match.id));

					// Delete match participants
					await tx.delete(match_players).where(eq(match_players.matchId, match.id));
				}

				// Delete all player course handicaps for this round (this was causing foreign key constraint errors)
				await tx.delete(player_course_handicaps).where(eq(player_course_handicaps.roundId, id));

				// Delete all matches in the round
				await tx.delete(matches).where(eq(matches.roundId, id));
			...
			await db.execute(
				`SELECT SETVAL('scores_id_seq', COALESCE((SELECT MAX(id) FROM scores), 0) + 1, false)`
			);
			await db.execute(
				`SELECT SETVAL('match_participants_id_seq', COALESCE((SELECT MAX(id) FROM match_participants), 0) + 1, false)`
			);
			await db.execute(
				`SELECT SETVAL('player_course_handicaps_id_seq', COALESCE((SELECT MAX(id) FROM player_course_handicaps), 0) + 1, false)`
			);
			await db.execute(
				`SELECT SETVAL('rounds_id_seq', COALESCE((SELECT MAX(id) FROM rounds), 0) + 1, false)`
			);
	...
	private async resetSequence(tableName: string) {
		...
		await db.execute(`ALTER SEQUENCE "${tableName}_id_seq" RESTART WITH ${maxId}`);
	}
	...
	async deleteMatch(id: number) {
		try {
			await db.transaction(async (tx) => {
				...
				// Delete all match participants
				await tx.delete(match_players).where(eq(match_players.matchId, id));
	...
	async getScore(matchId: number, holeNumber: number) {
		const [row] = await db
			.select()
			.from(scores)
			.where(and(eq(scores.matchId, matchId), eq(scores.holeNumber, holeNumber)));
		return row;
	}
	...
	async updateScore(id: number, data: Partial<any>) {
		const [row] = await db.update(scores).set(data).where(eq(scores.id, id)).returning();
		return row;
	}

	async updateScoreAndMatch(id: number, data: Partial<any>) {
		const [updatedScore] = await db.update(scores).set(data).where(eq(scores.id, id)).returning();
	...
	async getPlayerScoreById(id: number) {
		const [row] = await db.select().from(player_scores).where(eq(player_scores.id, id));
		return row;
	}

	async getPlayerScoresByMatch(matchId: number) {
		return db.select().from(player_scores).where(eq(player_scores.matchId, matchId));
	}

	async getPlayerScoresByPlayer(playerId: number) {
		return db.select().from(player_scores).where(eq(player_scores.playerId, playerId));
	}
	...
	async getPlayerScoresByPlayerAndMatch(playerId: number, matchId: number) {
		return db
			.select()
			.from(player_scores)
			.where(and(eq(player_scores.playerId, playerId), eq(player_scores.matchId, matchId)))
	...
	async updateTournament(id: number, data: Partial<any>) {
		const [row] = await db.update(tournament).set(data).where(eq(tournament.id, id)).returning();
		return row;
	}
	...
	async getTournamentHistoryEntry(id: number) {
		const [row] = await db.select().from(tournament_history).where(eq(tournament_history.id, id));
		return row;
	}

	async createTournamentHistoryEntry(data: any) {
		const [row] = await db.insert(tournament_history).values(data).returning();
		return row;
	}
	...
	async createPlayerMatchup(data: any) {
		...
		const [row] = await db.insert(player_matchups).values([data]).returning();
		return row;
	}
	...
	async getPlayerMatchups(playerId: number) {
		...
		return db
			...
			.where(or(eq(player_matchups.playerId, playerId), eq(player_matchups.opponentId, playerId)))
	...
	async getPlayerVsOpponentMatchups(playerId: number, opponentId: number) {
		return db
			...
			.where(
				or(
					and(eq(player_matchups.playerId, playerId), eq(player_matchups.opponentId, opponentId)),
					and(eq(player_matchups.playerId, opponentId), eq(player_matchups.opponentId, playerId))
				)
			)
			.orderBy(desc(player_matchups.createdAt));
	}
	...
	async getPlayerMatchupsStats(playerId: number) {
		...
		const wins = await db
			.select({ count: count() })
			.from(player_matchups)
			.where(and(eq(player_matchups.playerId, playerId), eq(player_matchups.result, 'win')));

		const losses = await db
			.select({ count: count() })
			.from(player_matchups)
			.where(and(eq(player_matchups.playerId, playerId), eq(player_matchups.result, 'loss')));

		const ties = await db
			.select({ count: count() })
			.from(player_matchups)
			.where(and(eq(player_matchups.playerId, playerId), eq(player_matchups.result, 'tie')));
	...
	async updatePlayerMatchTypeStats(
		playerId: number,
		matchType: string,
		result: 'win' | 'loss' | 'tie'
	) {
		...
		if (existing) {
			...
			const [row] = await db
				.update(player_match_type_stats)
				.set(updates)
				.where(
					and(
						eq(player_match_type_stats.playerId, playerId),
			...
			const [row] = await db.insert(player_match_type_stats).values(data).returning();
			return row;
		}
	}
	...
	async calculateAndUpdatePlayerStats(playerId: number, tournamentId: number) {
		...
		const careerTotals = allTournamentStats.reduce(
			(totals, tournStat) => {
				return {
	...
	async updateTournamentHistory(tournamentId: number) {
		...
		if (existingEntry) {
			...
			// Create new entry
			const [newEntry] = await db.insert(tournament_history).values(historyData).returning();
			return newEntry;
		}
	}
	...
	async saveBestBallScore(score: InsertBestBallScore) {
		...
		if (existingScore.length > 0) {
			...
			// Insert new score
			return db.insert(best_ball_player_scores).values(score).returning();
		}
	}
	...
	async updateBetType(id: number, data: Partial<BetType>): Promise<BetType | undefined> {
		const [betType] = await db.update(bet_types).set(data).where(eq(bet_types.id, id)).returning();
		return betType;
	}
	...
	async updateParlay(id: number, data: Partial<Parlay>): Promise<Parlay | undefined> {
		const [parlay] = await db.update(parlays).set(data).where(eq(parlays.id, id)).returning();
		return parlay;
	}
	...
	async getUserLedgerEntries(userId: number): Promise<LedgerEntry[]> {
		return db
			.select()
			.from(betting_ledger)
			.where(or(eq(betting_ledger.creditorId, userId), eq(betting_ledger.debtorId, userId)));
	}
	...
	async getLedgerEntriesBetweenUsers(user1Id: number, user2Id: number): Promise<LedgerEntry[]> {
		return db
			...
			.where(
				or(
					and(eq(betting_ledger.creditorId, user1Id), eq(betting_ledger.debtorId, user2Id)),
					and(eq(betting_ledger.creditorId, user2Id), eq(betting_ledger.debtorId, user1Id))
				)
			);
	}
	...
	async getUserBalance(userId: number): Promise<{ owed: number; owing: number; net: number }> {
		...
		const [creditorResult] = await db
			...
			.from(betting_ledger)
			.where(and(eq(betting_ledger.creditorId, userId), eq(betting_ledger.status, 'pending')));
		...
		const [debtorResult] = await db
			...
			.from(betting_ledger)
			.where(and(eq(betting_ledger.debtorId, userId), eq(betting_ledger.status, 'pending')));
