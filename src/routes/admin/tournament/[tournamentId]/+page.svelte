<script lang="ts">
	...
	onMount(async () => {
		...
		const { data: t, error: err1 } = await supabase
			...
			.eq('id', id)
			.single();
		const { data: r, error: err2 } = await supabase
			.from('rounds')
			.select('*')
			.eq('tournament_id', id);
	...
	async function saveEditRound() {
		...
		const { error: err } = await supabase
			.from('rounds')
			.update({ name: editRoundName.trim() })
...
<section class="mx-auto max-w-3xl p-4">
	...
	{:else}
		<form class="mb-4 flex gap-2" on:submit|preventDefault={addRound}>
			...
			/>
			<button class="rounded bg-blue-600 px-3 py-1 text-white" type="submit">Add Round</button>
		</form>
		{#if addError}
			<div class="mb-2 text-red-600">{addError}</div>
		{/if}
		<ul>
			{#each rounds as round}
				<li class="mb-2 flex items-center gap-2">
					{#if editingRoundId === round.id}
						<input class="rounded border px-2 py-1" bind:value={editRoundName} />
						<button
							class="rounded bg-blue-600 px-2 py-1 text-xs text-white"
							on:click={saveEditRound}>Save</button
						>
						...
						{/if}
					{:else}
						<a class="text-blue-700 underline" href={`/admin/round/${round.id}`}
							>{round.name || `Round ${round.id}`}</a
						>
						<button class="text-xs text-blue-600" on:click={() => startEditRound(round)}
							>Edit</button
						>
						<button class="text-xs text-red-600" on:click={() => deleteRound(round.id)}
							>Delete</button
						>
					{/if}
				</li>
			...
			{#each players as player}
				<li class="mb-2 flex items-center gap-2">
					...
					<button
						...
						on:click={() => removePlayerFromTournament(player.id)}>Remove</button
					>
				</li>
			{/each}
		</ul>
		<form class="mb-2 flex items-center gap-2" on:submit|preventDefault={addPlayerToTournament}>
			<select class="rounded border px-2 py-1" bind:value={addPlayerId}>
			...
			<button class="rounded bg-blue-600 px-2 py-1 text-xs text-white" type="submit"
				>Add Player</button
			>
		</form>
		{#if addPlayerError}
			<div class="mb-2 text-red-600">{addPlayerError}</div>
		{/if}
	{/if}
	...
	{:else}
		<ul class="mb-4">
			{#each teams as team}
				<li class="mb-2 flex items-center gap-2">
					<span>{team.name}</span>
					<button class="text-xs text-red-600" on:click={() => removeTeamFromTournament(team.id)}
						>Remove</button
					>
				</li>
			{/each}
		</ul>
		<form class="mb-2 flex items-center gap-2" on:submit|preventDefault={addTeamToTournament}>
			<input
				class="rounded border px-2 py-1"
		...
		{#if addTeamError}
			<div class="mb-2 text-red-600">{addTeamError}</div>
		{/if}
	{/if}
	...
	<ul class="mb-4">
		{#each players as player}
			<li class="mb-2 flex items-center gap-2">
				...
				<select
					class="rounded border px-2 py-1"
					bind:value={player.team_id}
					on:change={(e) => assignPlayerToTeam(player.id, e.target.value)}
				>
				...
				{#if player.team_id}
					<span class="text-xs text-gray-500"
						>Current: {teams.find((t) => t.id === player.team_id)?.name}</span
					>
				{/if}
			</li>
		{/each}
	...
	{#if assignPlayerTeamError}
		<div class="mb-2 text-red-600">{assignPlayerTeamError}</div>
	{/if}
	...
	{:else}
		{#each rounds as round}
			<div class="mb-6">
				...
				{#if addMatchError}
					<div class="mb-2 text-red-600">{addMatchError}</div>
				{/if}
				<ul>
					{#each matchesByRound[round.id] || [] as match}
						<li class="mb-2 flex flex-col gap-2 border-b pb-2">
							<div class="flex items-center gap-2">
								...
								<button
									class="text-xs text-red-600"
									on:click={() => deleteMatch(match.id, round.id)}>Delete</button
								>
							</div>
							<div class="flex items-center gap-2">
								...
								<select
									class="rounded border px-2 py-1"
									bind:value={match.team_a_id}
									on:change={(e) => assignTeamsToMatch(match.id, e.target.value, match.team_b_id)}
								>
								...
								<select
									class="rounded border px-2 py-1"
									bind:value={match.team_b_id}
									on:change={(e) => assignTeamsToMatch(match.id, match.team_a_id, e.target.value)}
								>
									<option value="">Select Team</option>
									{#each teams as team}
										<option value={team.id}>{team.name}</option>
	...
	{/if}
	{#if assignTeamError}
		<div class="mb-2 text-red-600">{assignTeamError}</div>
	{/if}
</section>
