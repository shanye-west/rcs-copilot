<script lang="ts">
	...
	async function addMatch() {
		...
		if (err) {
			...
			newMatchName = '';
			// Reload matches
			const { data: m, error: err2 } = await supabase
				.from('matches')
				.select('*')
	...
	async function saveEditMatch() {
		...
		const { error: err } = await supabase
			.from('matches')
			.update({ name: editMatchName.trim() })
...
<section class="mx-auto max-w-3xl p-4">
	...
	{:else}
		<form class="mb-4 flex gap-2" on:submit|preventDefault={addMatch}>
			...
			/>
			<button class="rounded bg-blue-600 px-3 py-1 text-white" type="submit">Add Match</button>
		</form>
		{#if addMatchError}
			...
			{#each matches as match}
				<li class="mb-2 flex items-center gap-2">
					{#if editingMatchId === match.id}
						<input class="rounded border px-2 py-1" bind:value={editMatchName} />
						<button
							class="rounded bg-blue-600 px-2 py-1 text-xs text-white"
							on:click={saveEditMatch}>Save</button
						>
						<button class="text-xs text-gray-600" on:click={cancelEditMatch}>Cancel</button>
						{#if editMatchError}
							<span class="text-xs text-red-600">{editMatchError}</span>
						{/if}
					{:else}
						<a class="text-blue-700 underline" href={`/admin/match/${match.id}`}
							>{match.name || `Match ${match.id}`}</a
						>
						<button class="text-xs text-blue-600" on:click={() => startEditMatch(match)}
							>Edit</button
						>
						<button class="text-xs text-red-600" on:click={() => deleteMatch(match.id)}
							>Delete</button
						>
					{/if}
				</li>
