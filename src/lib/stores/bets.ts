...
function createBetsStore() {
  ...
  return {
    ...
    // Create a new bet
    createBet: async (betData: Omit<Database['public']['Tables']['bets']['Insert'], 'id' | 'created_at' | 'updated_at'>) => {
      update(state => ({ ...state, loading: true, error: null }));
      try {
        const { data, error } = await supabase
          .from('bets')
      ...
      try {
        ...
        update(state => ({
          ...
          bets: state.bets.map(bet => 
            bet.id === betId ? { ...bet, ...updateData } : bet
          ),
          loading: false
        }));
    ...
    markAsPaid: async (betId: string) => {
      ...
      try {
        ...
        update(state => ({
          ...state,
          bets: state.bets.map(bet => 
            bet.id === betId ? { ...bet, is_paid: true } : bet
          ),
          loading: false
        }));
...
export const pendingIncomingBets = derived(
  [betsStore, auth],
  ([$betsStore, $auth]) => {
    if (!$auth.user) return [];
    return $betsStore.bets.filter(bet => 
      bet.opponent_id === $auth.user.id && 
      bet.status === BetStatus.PENDING
    );
  }
);

export const pendingOutgoingBets = derived(
  [betsStore, auth],
  ([$betsStore, $auth]) => {
    if (!$auth.user) return [];
    return $betsStore.bets.filter(bet => 
      bet.creator_id === $auth.user.id && 
      bet.status === BetStatus.PENDING
    );
  }
);

export const activeBets = derived(
  [betsStore, auth],
  ([$betsStore, $auth]) => {
    if (!$auth.user) return [];
    return $betsStore.bets.filter(bet => 
...
export const completedBets = derived(
  [betsStore, auth],
  ([$betsStore, $auth]) => {
    if (!$auth.user) return [];
    return $betsStore.bets.filter(bet => 
