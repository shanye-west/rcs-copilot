<script lang="ts">
  ...
  onMount(() => {
    if (!isOnline && unsyncedCount > 0) {
      displayToast(`You're offline with ${unsyncedCount} unsynced score${unsyncedCount === 1 ? '' : 's'}.`, 'error');
    }
  });
</script>
...
{#if showToast}
  <div class="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 transform">
    <div class={`rounded px-4 py-2 shadow-lg ${toastType === 'success' ? 'bg-green-500' : 'bg-red-500'} text-white`}>
      {toastMessage}
    </div>
  </div>
{/if}
