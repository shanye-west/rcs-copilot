...
<div class="mb-4">
	<h2 class="mb-2 text-center text-lg font-bold">1v1 Individual Match Scorecard</h2>
			...
			<thead class="bg-gray-50">
				<tr>
					...
					<th class="border bg-blue-50 px-2 py-1 text-center font-bold text-blue-700"
						>{players[0].player.username}</th
					>
					<th class="border bg-green-50 px-2 py-1 text-center font-bold text-green-700"
						>{players[1].player.username}</th
					>
				</tr>
				<tr>
					<th class="sticky left-0 z-10 border bg-gray-100 px-2 py-1 text-xs text-gray-400">Dots</th
					>
					<th class="border bg-blue-50 px-2 py-1 text-xs text-blue-500"
						>{holes.map((h) => getDots(players[0].player, h)).join(' ')}</th
					>
					<th class="border bg-green-50 px-2 py-1 text-xs text-green-600"
					...
					<tr>
						...
						{#each players as p (p.player.id)}
							<td class="border px-2 py-1 text-center">
								...
								{/if}
								<div class="mt-1 text-xs text-gray-400">{getDots(p.player, hole)}</div>
							</td>
						{/each}
					</tr>
				{/each}
